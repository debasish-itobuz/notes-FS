...
const Home = () => {
    ...
    const getAll = async () => {
        try {
            const res = await axios.get(`http://localhost:8001/note/searchSortPaginateNote?sortField=${sortField}&sortOrder=${sortOrder}&searchText=${debounce}`, {
                headers: {
...
//     const getAll = async () => {
//         try {
//             const res = await axios.get(`http://localhost:8001/note/searchSortPaginateNote?sortField=${sortField}&sortOrder=${sortOrder}&searchText=${debouncedText}`, {
//                 headers: {
//                     Authorization: `Bearer ${accessToken}`
//                 }
//             })
...
//     useEffect(() => {
//         getAll()
//     }, [debouncedText, sortField, sortOrder])
...
//                 <div className="p-4 text-gray-600 dark:text-gray-300 outline-none focus:outline-none">
//                     <div className="relative flex">
//                         <select className="bg-white text-gray-600 h-10 px-8 rounded-l-full text-sm focus:outline-none border-2 border-gray-500 dark:border-gray-600 border-r-1 cursor-pointer max-h-10 overflow-y-hidden" onChange={handleSortChange}>
...
//             <div className='flex justify-center flex-wrap gap-6 mt-20'>
//                 {notes.map((item, index) => (
//                     <Card key={index} title={item.title} content={item.content} noteId={item._id} getAll={getAll} />
//                 ))}
//             </div>
//             <Footer />
//         </>
